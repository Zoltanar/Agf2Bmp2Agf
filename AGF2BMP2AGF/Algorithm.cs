...
using static EushullyExtractionUtils.Operation;
...
namespace AGF2BMP2AGF
{
    public static class Algorithm
    {
        ...
        public static bool Unpack(string inputFile, string outputFile, ProcessData processData)
        {
            ...
            try
            {
                Operation.ReadToStructure(fileStream, out AGFHDR hdr, Marshal.SizeOf<AGFHDR>());
                if (hdr.type != AgfType24Bit && hdr.type != AgfType32Bit)
                {
                    ConsoleUtils.PrintError($"File {inputFile} was of unsupported type (possibly an MPEG): {hdr.type}");
                    return false;
                }
        ...
        public static Stream OpenFileOrDie(string filename, FileMode fileMode)
        {
