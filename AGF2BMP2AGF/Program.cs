...
public static class Program
{
    ...
    private static int Main()
    {
        ...
        {
            ConsoleUtils.PrintError(ex.ToString());
        }
#if DEBUG
    ...
    private static int Main2(string[] argv)
    {
        ...
        {
            ConsoleUtils.PrintError(runParameters.ErrorMessage);
            PrintHelp(argv[0]);
            return -1;
        }
        ConsoleUtils.PrintWarning(runParameters.Description);
        ...
        if (errors > 0) ConsoleUtils.PrintError($"{completedString} ({errors} errors)");
        else ConsoleUtils.Print(completedString);
        return errors;
    }
        ...
        ConvertFileData file, ref int errors)
    {
        if (fileCount > 1)
        {
            if (Console.KeyAvailable)
            {
                ...
                Console.ReadKey(true);
                ConsoleUtils.PrintWarning("Key pressed, press Escape to stop or any other key to continue...");
                var key = Console.ReadKey(true);
                if (key.Key == ConsoleKey.Escape) return false;
            }
            if (runParameters.LogErrorsOnly) ConsoleUtils.Print(ConsoleUtils.WarningColor, $"Processing file {file.Index.ToString(formatString)}/{fileCount}", true);
            else ConsoleUtils.PrintWarning(file.GetDescription(fileCount, formatString));
        }
        ...
        {
            errors++;
            ConsoleUtils.PrintError($"\t{file.GetDescription(fileCount, formatString)} - Failed: {ex}");
        }
    ...
    private static void ProcessFileParallel(ConvertFileData file, bool logErrorsOnly)
    {
        try
        {
            ...
            if (!success || !logErrorsOnly) ConsoleUtils.Print(success ? ConsoleUtils.SuccessColor : ConsoleUtils.ErrorColor, $"\t{file.GetDescription(null, null)}");
        }
        catch (Exception ex)
        {
            ConsoleUtils.PrintError($"\t{file.GetDescription(null, null)} - Failed: {ex}");
        }
    }
...
Usage: {thisFile} [switches] <input> [output] [original_agf]
  ...
  original_agf:  not required if unpacking.
	...
	Original AGF files are required to pack BMP files back into same format.";
        ...
        ConsoleUtils.PrintWarning(help);
    }
}
